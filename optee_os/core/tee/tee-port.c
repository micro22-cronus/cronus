...
void call_rcu(struct rcu_head *head, rcu_callback_t func) NO_IMPL;
...
// srcu
int __srcu_read_lock(struct srcu_struct *ssp) NO_IMPL;
void __srcu_read_unlock(struct srcu_struct *ssp, int idx) NO_IMPL
...
void synchronize_rcu(void) NO_IMPL;
...
struct file *shmem_file_setup(const char *name,
                              loff_t size, unsigned long flags) NO_IMPL;
struct page *shmem_read_mapping_page_gfp(struct address_space *mapping,
                                         pgoff_t index, gfp_t gfp_mask) NO_IMPL_0;

int rcuwait_wake_up(struct rcuwait *w) NO_IMPL;
void kvfree_call_rcu(struct rcu_head *head, rcu_callback_t func) NO_IMPL;
